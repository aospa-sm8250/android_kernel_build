...
export KERNEL_PREBUILT_DIR=$(readlink -m ${COMMON_OUT_DIR}/ship_prebuilt)
...
make_defconfig()
{
	if [ -z "${SKIP_DEFCONFIG}" ] ; then
		...
		set -x
		(cd ${KERNEL_DIR} && \
		${MAKE_PATH}make O=${OUT_DIR} ${MAKE_ARGS} HOSTCFLAGS="${TARGET_INCLUDES}" HOSTLDFLAGS="${TARGET_LINCLUDES}" ARCH=${ARCH} ${real_cc} ${DEFCONFIG})
		set +x
	fi
